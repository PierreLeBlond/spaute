<script lang="ts">
  ...
  export let checkedLabel: string | null = null;
  export let uncheckedLabel: string | null = null;
...
<div
  ...
  class:pb-4={checkedLabel}
>
  <div
    class="relative grid grid-cols-5 items-center justify-center"
    class:grid-cols-5={checkedLabel}
  >
    {#if checkedLabel}
      <label
        for="{field}-input"
        class="col-span-3 text-xs">{$checked ? checkedLabel : uncheckedLabel}</label
      >
    {/if}
    <input
      form={$formId}
