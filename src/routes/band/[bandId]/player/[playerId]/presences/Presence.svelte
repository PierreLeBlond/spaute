<script lang="ts">
  ...
  const updatePresence = async (value: boolean) => {
    await fetch(`./presence/${presence.id}`, {
      method: "PATCH",
      body: JSON.stringify({
        data: { value },
      }),
  ...
  let disabled = true;
  const onPresenceChange = async (value: boolean) => {
    if (disabled) {
      return;
    }
  ...
  $: onPresenceChange(presence.value);
